...
def run_simulation_w_points(expressions_to_vary,multiplier_values,simModel,DataFolderName,FigureFolderName,folderName,nTries=1000):
    ...
    biomarkerNames = ['APD90', 'APD60', 'APD40', 'APA', 'TRI60', 'TRI40', '90PERCENT', '90to90', 'PeakDist', 'EAD', 'Alternan','Pro-Arrhythmic']
    ...
    for iTry in tqdm(range(nTries)):
        ...
        for iTrace, Trace in enumerate(AP_interpolators):
            for jTrace in range(iTrace + 1, len(AP_interpolators)):
                ...
                if np.any(np.abs(np.diff(Trace - AP_interpolators[jTrace])) >= 1):
                    ...
                    countAlternans += 1
                    newRow = [iTry] + expression_gains + [np.nan] * len(biomarkerNames[:-3]) + [EAD, isAnAlternan, isProArrhythmic]
        ...
        if not isAnAlternan:
            ...
            newRow = [iTry] + expression_gains + [APD90, APD60, APD40, maxVoltage - minVoltageAfterPeak, triangulationSlope60, triangulationSlope40, repolarisationVoltage90, APD90to90, time_between_peaks, EAD, isAnAlternan, isProArrhythmic]
            biomarkers.loc[iTry] = newRow
        ################################################################################################################
    ...
    for iBiomarker, biomarker in enumerate(biomarkerNames):
        ...
        for iGain, gainName in enumerate(gains_to_vary):
            axs1[iAxs].scatter(stable[gainName], stable[biomarker], s=5, label='Multiplier on '+ gain_labels[iGain])
        ...
        axs1[iAxs].set_xscale('symlog', base=2, linthresh=0.0625)
        axs1[iAxs].set_xticks([0.0625, 0.125, 0.25, 0.5, 1, 2, 4, 8, 16])
        axs1[iAxs].set_xlim(0.0625, 16)
        if iAxs == 2:
            axs1[iAxs].legend()
        iAxs += 1
    ...
    ####################################################################################################################
    # plot histograms of the biomarkers (9 numerical)
    fig2, axs2 = plt.subplots(3, 3, figsize=(15, 10))
    axs2 = axs2.ravel()
    iAxs = 0
    for iBiomarker, biomarker in enumerate(biomarkerNames):
        if biomarker == 'Alternan' or biomarker == 'EAD' or biomarker == 'Pro-Arrhythmic':
    ...
    axs3[0].set_xscale('symlog', base=2, linthresh=0.0625)
    axs3[0].set_xticks([0.0625,0.125, 0.25, 0.5, 1, 2, 4, 8, 16])
    axs3[0].set_yscale('symlog', base=2, linthresh=0.0625)
    axs3[0].set_yticks([0.0625, 0.125, 0.25, 0.5, 1, 2, 4, 8, 16])
    axs3[0].set_xlim(0.0625, 16)
    axs3[0].set_ylim(0.0625, 16)
    axs3[0].legend(loc='best')
    for iTry in range(nTries):
        axs3[1].plot(simulationResults['Time'][iTry] / 1000, simulationResults['Voltage'][iTry], color='k',linewidth=0.5, alpha=0.5)
    for iArrythmic in range(len(simRes_pro_arrhythmic['Index'])):
        axs3[1].plot(simRes_pro_arrhythmic['Time'][iArrythmic] / 1000, simRes_pro_arrhythmic['Voltage'][iArrythmic], linewidth=0.5, color='magenta')
...
if __name__ == '__main__':
    ...
    if isATest:
        nTries = 30  # number of simulations
    else:
        nTries = 3000
...
########################################################################################################################
    ...
    covars = pickle.load(open('Pickles/IKr_IKs_ICaL_INa_Ito_covariance.pkl', 'rb'))
    Sigma_all = covars
    # we only want the diagonal element from the
    if isATest:
        sigmas = [1.5]*len(expressions_to_vary) # for testing pro-arrythimic highlighting - make sure to have large sigmas to generate some alternans
    else:
    #     get the diagonal elements of the covariance matrix
        sigmas = np.sqrt(np.diag(Sigma_all))*1.5 # 1.5 is a scaling factor to make the variation more pronounced
    print('Expressions to vary: ', expressions_to_vary)
    print('Sigmas: ', sigmas)
...
########################################################################################################################
    print('Covaried conductances for all principal genes')
    # create folder to store selected simulation outputs
    # baseline conductances of the currents of interest
    folderName = 'all_cotranscripted'
    if isATest:
        folderName = folderName + '_test'
    if not os.path.exists(DataFolderName + '/' + folderName):
        os.makedirs(DataFolderName + '/' + folderName)
    ...
    sampled_from_normal = np.random.multivariate_normal(mean=means, cov=Sigma_all, size=nTries)
    ...
    ########################################################################################################################
    print('Covaried conductances for all principal genes (converted from the co-transcription samples)')
    # create folder to store selected simulation outputs
    folderName = 'all_cotranslated'
    if isATest:
        folderName = folderName + '_test'
    if not os.path.exists(DataFolderName + '/' + folderName):
        os.makedirs(DataFolderName + '/' + folderName)
...
########################################################################################################################
    print('Dependent variation of conductances for all principal genes')
    # create folder to store selected simulation outputs
    folderName = 'all_dependent'
    if isATest:
        folderName = folderName + '_test'
    if not os.path.exists(DataFolderName + '/' + folderName):
        os.makedirs(DataFolderName + '/' + folderName)
    ...
    gain_values = np.concatenate((gain_values, gain_values, gain_values, gain_values, gain_values), axis=1)
